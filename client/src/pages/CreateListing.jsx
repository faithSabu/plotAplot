...
export default function CreateListing() {
  ...
  const handleImageSubmit = async (e) => {
    if (files.length > 0 && files.length < 7 && formData.imageURLs.length < 7) {
      ...
      setImageUploadErr("Should only have 1-6 images");
    }
  };
  ...
  const handleRemoveImage = (index) => {
    const newImageArr = formData.imageURLs.filter((_, i) => i !== index);
    setFormData({
      ...formData,
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <button
            ...
            className="p-3 bg-slate-700 text-white rounded-lg uppercase mt-2 hover:opacity-90 disabled:opacity-80 disabled:cursor-not-allowed"
          >
